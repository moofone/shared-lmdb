...
| `Circular { max_count }` | Keeps only the newest `max_count` samples per series_key. Oldest samples are deleted. |
| `MaxAgeMs { max_age_ms }` | Keeps only samples whose timestamp is within `max_age_ms` milliseconds of the newest sample for that series_key. |
...
- `upsert_sample`
- `upsert_batch_refs` (and by extension `upsert_batch`)
- `replace_history`
...
1. Build the series_key prefix bytes via `series_prefix(series_key)`.
...
1. Build the series_key prefix bytes.
...
- **Default (text keys):** `format!("{series_key}:{timestamp_ms:020}")` -- zero-padded to 20 digits, so lexicographic order matches chronological order.
- **`binary-keys` feature:** `series_key_bytes | separator | timestamp_ms.to_be_bytes()` -- big-endian encoding preserves ordering.
