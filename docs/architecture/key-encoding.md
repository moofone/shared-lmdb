...
Timeseries keys in `shared-lmdb` are composed of a **series_key** string and a **timestamp** (milliseconds, `u64`). The encoding is selected at compile time via the `binary-keys` Cargo feature.
...
```rust
// Format: "{series_key}:{timestamp_ms:020}" as UTF-8 bytes
let key = format!("{series_key}:{timestamp_ms:020}").into_bytes();

// Example: series_key = "BTCUSDT", ts = 1_234_567_890
// b"BTCUSDT:00000000001234567890"
```

- Separator: `b':'` (0x3A)
- Timestamp: zero-padded to 20 decimal digits
- Lexicographic byte order matches chronological order (monotonically increasing timestamp within a series_key)

### Binary mode (`feature = "binary-keys"`)

```rust
// Format: series_key_bytes | 0x7C | timestamp_ms.to_be_bytes()
let mut out = Vec::with_capacity(series_key.len() + 1 + 8);
out.extend_from_slice(series_key.as_bytes());
out.push(b'|');
out.extend_from_slice(&timestamp_ms.to_be_bytes());

// Example: series_key = "BTCUSDT", ts = 42
...
```rust
// Compose a full key from series_key and timestamp.
fn encode_key(series_key: &str, timestamp_ms: u64) -> Vec<u8>;

// Return series_key bytes + separator byte, used as a prefix for LMDB prefix iteration.
...
Both modes construct the scan prefix as `series_key.as_bytes() + [separator_byte]`. This is passed to `heed::Database::prefix_iter` to retrieve all entries for a given series_key, which underpins range queries, rotation policies (`Circular`, `MaxAgeMs`), and enumeration endpoints.
...
- `binary_key_parse_rejects_invalid_prefix` -- encoding with one series_key, parsing with another returns `InvalidKey`
